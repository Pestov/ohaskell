...
И ещё об ожиданиях. Не ждите от этой книги всеохватной полноты рассмотрения Haskell и его экосистемы. Кроме того, это не справочник. Я не буду копировать сюда всё содержимое официального сайта Haskell[2](#ftn2) или переводить на русский язык стандарт Haskell 2010[3](#ftn3). Также здесь не приводится информация, которую госпожа Википедия выдаст вам в один момент (например, повествование об истории языка[4](#ftn4)).
...
Создаём настоящий проект и готовимся к реальной работе.

## <a name="Создаём проект">Создаём проект

Мы не можем начать изучение языка без полигона. Поэтому скачайте и установите Haskell Platform[5](#ftn5).
...
В реальной работе вы не будете создавать файлик Main.hs[7](#ftn7) на рабочем столе для последующего скармливания его компилятору. Напротив, вы создадите нормальный рабочий проект с логичной внутренней структурой. Так давайте и создадим такой с самого начала. А поможет нам в этом удобная утилита из Haskell Platform с необычным названием cabal[8](#ftn8).

Утилита cabal предназначена для сборки проектов. Уверен, вы слышали о вещах типа cmake или qmake, так вот воспринимайте cabal как «cmake специально для Haskell».
...
Как уже было упомянуто, в корневом каталоге нашего проекта появились два новых файла, Real.cabal и Setup.hs. Второй файл нам не так интересен[9](#ftn9), а вот первый — это и есть сборочный файл нашего проекта. Откроем его:
 
```haskell
...
Нам необходимо раскомментировать эту строку и прописать в ней имя файла Main.hs, содержащего функцию main[10](#ftn10):
